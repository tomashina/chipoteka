<?php
class ControllerExtensioncompgapro extends Controller {
	private $error = array();  
	private $modpath = 'extension/compgapro';
	private $modtpl = 'extension/compgapro.tpl';
	private $modssl = 'SSL';
	private $token_str = ''; 
	...
	public function __construct($registry) {
		...
		if(substr(VERSION,0,3)>='3.0' || substr(VERSION,0,3)=='2.3') { 
 			$this->modpath = 'extension/compgapro';
 			$this->modtpl = 'extension/compgapro';
	...
	public function index() {
		...
		$this->model_extension_compgapro->checkdb();
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_extension_compgapro->add($this->request->post);
		}
		...
		$rs = $this->model_extension_compgapro->getdata();
