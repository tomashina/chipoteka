<?php
	...
	if ( (float)$product_info['special'] && ($this->config->get('salebadge_status')) ) {
        ...
        if ($product_info['price_last_30'] != '0.0000' && $product_info['price_last_30'] < $product_info['price_2']) {

            if ($this->config->get('salebadge_status') == '2') {

                $data['sale_percent'] = number_format(((($this->tax->calculate($product_info['price_last_30'], $product_info['tax_class_id'], $this->config->get('config_tax')))-($this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax'))))/(($this->tax->calculate($product_info['price_last_30'], $product_info['tax_class_id'], $this->config->get('config_tax')))/100)), 0, ',', '.');
                ...
                if($data['sale_percent'] > 3){
                    ...
                    if($data['razlika_broj'] > 100){
                        $data['sale_badge'] = $this->currency->format($this->tax->calculate($product_info['price_last_30'], $product_info['tax_class_id'], $this->config->get('config_tax')) -  $this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
                    }
                    else{
                        $data['sale_badge'] = '-' .number_format(((($this->tax->calculate($product_info['price_last_30'], $product_info['tax_class_id'], $this->config->get('config_tax')))-($this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax'))))/(($this->tax->calculate($product_info['price_last_30'], $product_info['tax_class_id'], $this->config->get('config_tax')))/100)), 0, ',', '.'). '%';
                    }
            ...
            if ($this->config->get('salebadge_status') == '2') {
                ...
                if($data['sale_percent'] > 3){
                    ...
                    //provjera razlike  veće od 100 kn prikaži cifru inače postotak
                    if($data['razlika_broj'] > 100){
                        $data['sale_badge'] = $this->currency->format($this->tax->calculate($product_info['price_2'], $product_info['tax_class_id'], $this->config->get('config_tax')) -  $this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
                    }
                    else{
