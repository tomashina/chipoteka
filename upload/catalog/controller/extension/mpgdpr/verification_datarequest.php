...
class ControllerExtensionMpGdprVerificationDataRequest extends \Mpgdpr\Controller {
	...
	public function index() {
		...
		$this->load->language($this->extension_path . 'mpgdpr/verification_datarequest');
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link($this->extension_path . 'mpgdpr/verification_datarequest', '', true)
		];
		...
		$this->response->setOutput($this->load->view($this->extension_path . 'mpgdpr/verification_datarequest', $data));
	}
	...
	public function verification() {
		...
		if (!$json) {
			// validate code here
			$request_info = $this->{'model_'. $this->extension_model .'mpgdpr_mpgdpr'}->getPersonalDataRequestByCode($this->request->post['code']);
		...
		if (!$json) {
			...
			if (strtotime($today) > strtotime($request_info['expire_on']) && $request_info['status'] == \Mpgdpr\Mpgdpr :: REQUESTACCESS_AWATING ) {
				...
				$this->{'model_'. $this->extension_model .'mpgdpr_mpgdpr'}->updatePersonalDataRequestStatus($request_info['mpgdpr_datarequest_id'], \Mpgdpr\Mpgdpr :: REQUESTACCESS_EXPIRE);
		...
		if (!$json) {
			...
			if ($request_info['status'] == \Mpgdpr\Mpgdpr :: REQUESTACCESS_AWATING ) {
				...
				$this->{'model_'. $this->extension_model .'mpgdpr_mpgdpr'}->updatePersonalDataRequestStatus($request_info['mpgdpr_datarequest_id'], \Mpgdpr\Mpgdpr :: REQUESTACCESS_CONFIRMED);
	...
	public function resentCode() {
		...
		if (!$json) {
			$success = $this->{'model_'. $this->extension_model .'mpgdpr_mpgdpr'}->resentPersonalDataRequestCode((int)$this->request->get['mpgdpr_datarequest_id'], $this->customer->getEmail());
			if ($success) {
