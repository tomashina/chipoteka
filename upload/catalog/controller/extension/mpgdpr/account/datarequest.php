...
class ControllerExtensionMpGdprAccountDataRequest extends \Mpgdpr\Controller {
	...
	public function index() {
		if ($this->config->get('mpgdpr_login_gdprforms') && !$this->customer->getId()) {
			$this->session->data['redirect'] = $this->url->link($this->extension_path . 'mpgdpr/account/datarequest', '', true);
		...
		$this->load->language($this->extension_path . 'mpgdpr/datarequest');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$mpgdpr_datarequest_id = $this->{'model_'. $this->extension_model .'mpgdpr_mpgdpr'}->addPersonalDataRequest($request_data);
			...
			// 01-05-2022: updation start
			$mpgdpr_requestlist_id = $this->{'model_'. $this->extension_model .'mpgdpr_mpgdpr'}->addRequest(\Mpgdpr\Mpgdpr :: CODEREQUESTPERSONALDATA, $request_data);
			// 01-05-2022: updation end
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link($this->extension_path . 'mpgdpr/account/datarequest', '' .  $url, true)
		];
		...
		// 01-05-2022: updation start
		$data['text_gdpr_datarequest_list'] = $this->language->get('text_gdpr_datarequest_list');
		...
		$data['action'] = $this->url->link($this->extension_path . 'mpgdpr/account/datarequest', '', true);
		...
		$requestaccessdata_total = $this->{'model_'. $this->extension_model .'mpgdpr_mpgdpr'}->getTotalPersonalDataRequests($this->customer->getId(), $filter_data);

		$results = $this->{'model_'. $this->extension_model .'mpgdpr_mpgdpr'}->getPersonalDataRequests($this->customer->getId(), $filter_data);
		...
		foreach ($results as $result) {
			...
			if ($result['status'] == \Mpgdpr\Mpgdpr :: REQUESTACCESS_AWATING && !($today > strtotime($result['expire_on']))) {
				$approve = $this->url->link($this->extension_path . 'mpgdpr/verification_datarequest', '', true);
			}
			$resentcode = '';
			if ($result['status'] == \Mpgdpr\Mpgdpr :: REQUESTACCESS_AWATING && ($today > strtotime($result['expire_on']))) {
				$resentcode = $this->url->link($this->extension_path . 'mpgdpr/verification_datarequest/resentCode', 'mpgdpr_datarequest_id='.$result['mpgdpr_datarequest_id'].'&o=1', true);
			}
		...
		$data['sort_datarequest_id'] = $this->url->link($this->extension_path . 'mpgdpr/account/datarequest',  '&sort=dr.mpgdpr_datarequest_id' . $url, true);
		$data['sort_status'] = $this->url->link($this->extension_path . 'mpgdpr/account/datarequest',  '&sort=dr.status' . $url, true);
		$data['sort_date_send'] = $this->url->link($this->extension_path . 'mpgdpr/account/datarequest',  '&sort=dr.date_send' . $url, true);
		$data['sort_date_added'] = $this->url->link($this->extension_path . 'mpgdpr/account/datarequest',  '&sort=dr.date_added' . $url, true);
		$data['sort_expire_on'] = $this->url->link($this->extension_path . 'mpgdpr/account/datarequest',  '&sort=dr.expire_on' . $url, true);
		...
		$pagination->url = $this->url->link($this->extension_path . 'mpgdpr/account/datarequest', $url . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($requestaccessdata_total) ? (($page - 1) * $limit) + 1 : 0, ((($page - 1) * $limit) > ($requestaccessdata_total - $limit)) ? $requestaccessdata_total : ((($page - 1) * $limit) + $limit), $requestaccessdata_total, ceil($requestaccessdata_total / $limit));
		...
		$this->response->setOutput($this->viewLoad($this->extension_path . 'mpgdpr/account/datarequest', $data));
	}
	...
	private function validate() {
		...
		if (!isset($this->error['email'])) {
			...
			if ($customer_id && !isset($this->error['warning'])) {
				...
				$total_requests = $this->{'model_'. $this->extension_model .'mpgdpr_mpgdpr'}->getTodayPersonalDataRequest($customer_id);
