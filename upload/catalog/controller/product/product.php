...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
            ...
            $product_info['meta_description'] = str_replace('"', '', $product_info['meta_description']);
			...
			if ($product_info['image']) {
                ...
                $data['slika'] = 'https://www.chipoteka.hr/image/'.$product_info['image'];
            ...
            if ($product_info['image']) {
                $this->document->addOGMeta('property="og:image"', 'https://www.chipoteka.hr/image/'.$product_info['image'] );



                $this->document->addOGMeta('property="twitter:image"', 'https://www.chipoteka.hr/image/'.$product_info['image'] );
            ...
            foreach ($results as $result) {
                $this->document->addOGMeta( 'property="og:image"', 'https://www.chipoteka.hr/image/'.$result['image']);
                $this->document->addOGMeta( 'property="twitter:image"', 'https://www.chipoteka.hr/image/'.$result['image']);
                $this->document->addOGMeta('property="og:image:width"', '600');
                $this->document->addOGMeta('property="og:image:height"', '600');
            }
            ...
            if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
                ...
                if($this->session->data['currency']=='HRK'){
                    ...
                    $data['priceeur_2'] = $this->currency->format($this->tax->calculate($product_info['price_2'], $product_info['tax_class_id'], $this->config->get('config_tax')), 'HRK');
            ...
            if ($product_info['price_last_30'] != '0.0000') {
                ...
                if($this->session->data['currency']=='HRK'){
                    ...
                    $data['priceeur_last_30'] = $this->currency->format($this->tax->calculate($product_info['price_last_30'], $product_info['tax_class_id'], $this->config->get('config_tax')), 'HRK');
                }
            }
            else{
            ...
            $this->load->model('catalog/category');
            $temp_cat = $this->model_catalog_category->getLowestTierSubcategory($this->request->get['product_id']);
            $this->response->redirect($this->url->link('product/category&path='.$temp_cat['category_id'], '', true));
