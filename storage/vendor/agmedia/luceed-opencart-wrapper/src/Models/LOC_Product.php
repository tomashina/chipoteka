...
class LOC_Product
{
    ...
    public function checkDiff()
    {
        ...
        $this->products_to_add = $this->getProducts()->whereIn('artikl_uid', $list_diff);
    ...
    public function sortForUpdate()
    {
        ...
        $this->products_to_add = $this->getProducts()->whereIn('artikl_uid', $this->existing);
    ...
    public function update(string $type = 'all')
    {
        ...
        foreach ($this->products_to_add as $item) {
            ...
            $query_str       .= '("' . $item->artikl_uid . '", ' . $item->mpc . ', ' . $stock . ', ' . $stock_status_id . '),';
        }
    ...
    private function updateOptionsPrices()
    {
        ...
        $query_str = '';
        $uids = $this->products_to_add->pluck('artikl_uid')->flatten();
        $products = Product::whereIn('model', $uids)->with('options')->get();

        foreach ($products as $product) {
            $new_price = $this->products_to_add->where('artikl_uid', $product->model)->first()->mpc;
    ...
    public function make($product): array
    {
        ...
        $scale         = AttributeHelper::resolveScale($this->product->atributi);

        $mock_stock = rand(0, 20);
        $mock_stock_status = $mock_stock ? agconf('import.default_stock_full') : agconf('import.default_stock_empty');

        $prod = [
            'model'               => $this->product->artikl_uid,
            'sku'                 => $this->product->artikl_uid,
            'upc'                 => '',
            'ean'                 => '',
            'jan'                 => '',
            'isbn'                => '',
            ...
            'scale'               => $scale,
            'active_min'          => AttributeHelper::resolveMin($this->product->atributi),
            'shipping'            => 1,
            ...
            'download'            => '',
            'related'             => '',
            'image'               => ! empty($this->product->dokumenti) ? $this->getImagePath() : 'placeholder.png',
            'points'              => '',
            'product_store'       => [0 => 0],
    ...
    {
        if (isset($this->product->proizvodac)) {
            $manufacturer = Manufacturer::where('luceed_uid', $this->product->proizvodac)->first();
     ...
     * If "grupa_artikla_uid" luceed tag is not found in opencart DB,
    ...
    private function getCategories()
    {
        ...
        $actual   = Category::where('luceed_uid', $this->product->grupa_artikla_uid)->first();
